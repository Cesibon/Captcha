...
class Captcha
{
    ...
    public static function set($config = array())
    {
        ...
        $bg_path = dirname(__DIR__) . '/backgrounds/';
        $font_path = dirname(__DIR__) . '/fonts/';
        ...
        $_SESSION['_CAPTCHA']['config'] = serialize($config);

        return array(
            'code' => $config['code'],
            'image_src' => self::getImageData($config),
        );

    }

    private static function getImageData($config)
    {

        if (!$config) exit();

        // Pick random background, get info, and start captcha
        $background = $config['backgrounds'][mt_rand(0, count($config['backgrounds']) - 1)];
        list($bg_width, $bg_height, $bg_type, $bg_attr) = getimagesize($background);

        $captcha = imagecreatefrompng($background);

        $color = self::hex2rgb($config['color']);
        $color = imagecolorallocate($captcha, $color['r'], $color['g'], $color['b']);

        // Determine text angle
        $angle = mt_rand($config['angle_min'], $config['angle_max']) * (mt_rand(0, 1) == 1 ? -1 : 1);

        // Select font randomly
        $font = $config['fonts'][mt_rand(0, count($config['fonts']) - 1)];
        ...
        //Set the font size.
        $font_size = mt_rand($config['min_font_size'], $config['max_font_size']);
        $text_box_size = imagettfbbox($font_size, $angle, $font, $config['code']);
        ...
        // Draw shadow
        if ($config['shadow']) {
            $shadow_color = self::hex2rgb($config['shadow_color']);
            $shadow_color = imagecolorallocate($captcha, $shadow_color['r'], $shadow_color['g'], $shadow_color['b']);
            imagettftext($captcha, $font_size, $angle, $text_pos_x + $config['shadow_offset_x'], $text_pos_y + $config['shadow_offset_y'], $shadow_color, $font, $config['code']);
        }
